...
AND ti.Fecha BETWEEN '2023-01-01' AND '2023-12-31';
...
/*7 nombre*/
SELECT Nombre, idEmpleadoR AS idEmpleado, SUM(CAST(Costo AS DECIMAL)) AS ganancia
FROM pepega_autoshop.trabajoindividual
JOIN pepega_autoshop.trabajo ON trabajoindividual.idTrabajo = trabajo.idTrabajo
JOIN pepega_autoshop.empleado ON trabajoindividual.idEmpleadoR = empleado.Matricula
WHERE pepega_autoshop.trabajoindividual.Fecha BETWEEN '2023-01-01' AND '2023-07-31'
GROUP BY Nombre, idEmpleadoR
...
/*10*/
SELECT d.NombreDepartamento, e.idDepartamento, SUM(vi.Cantidad) AS Reparaciones, ti.idTrabajo
...
JOIN pepega_autoshop.departamento AS d ON d.idDepartamento = e.idDepartamento 
...
/*15*/
WITH NReparaciones AS (
SELECT NombreDepartamento, COUNT(idTrabajoIndividual) as Reparaciones
FROM pepega_autoshop.trabajoindividual, pepega_autoshop.empleado, pepega_autoshop.departamento
WHERE trabajoindividual.idEmpleadoR = empleado.Matricula AND departamento.idDepartamento = empleado.idDepartamento
AND Fecha BETWEEN '2023-01-01' AND '2023-01-20'
...
SELECT NombreDepartamento, SUM(Consumo) as Consumo
FROM pepega_autoshop.trabajoindividual, pepega_autoshop.empleado, ConsumoRefacciones, pepega_autoshop.departamento
WHERE trabajoindividual.idEmpleadoR = empleado.Matricula 
AND trabajoindividual.idVenta = ConsumoRefacciones.idVenta AND departamento.idDepartamento = empleado.idDepartamento
GROUP BY departamento.idDepartamento;
...
/*18*/
SELECT Matricula, Nombre, Apellido1, Apellido2, NombreDepartamento 
FROM pepega_autoshop.empleado, pepega_autoshop.departamento
...
/*20*/
SELECT DISTINCT cliente.idCliente, Nombre, Apellido1, Apellido2
FROM pepega_autoshop.diagnostico, pepega_autoshop.vehiculo, pepega_autoshop.cliente
WHERE diagnostico.placa = vehiculo.Placa AND cliente.idCliente = vehiculo.idCliente
AND idDiagnostico NOT IN (SELECT idDiagnostico FROM pepega_autoshop.trabajo)
...
GROUP BY idCliente)
SELECT cliente.idCliente, Nombre, Apellido1, Apellido2, Gasto FROM ClienteGasto, pepega_autoshop.cliente
WHERE cliente.idCliente = ClienteGasto.idCliente
AND Gasto = (SELECT MAX(Gasto) FROM ClienteGasto AS SubConsulta);
...
/*23*/
SELECT refacciones.idProducto, NombreProducto, ROUND(SUM(Precio),2) as Total 
FROM pepega_autoshop.ventaindividual, pepega_autoshop.refacciones
WHERE refacciones.idProducto = ventaindividual.idProducto
GROUP BY refacciones.idProducto
...
/*24 CHECK*/
WITH DepartamentoGarantias AS (
SELECT empleado.idDepartamento, COUNT(trabajoindividual.idTrabajoIndividual) as Count, SUM(trabajoindividual.Costo) AS Gasto
...
SELECT NombreDepartamento, Count, Gasto FROM DepartamentoGarantias, pepega_autoshop.departamento
WHERE DepartamentoGarantias.idDepartamento = departamento.idDepartamento
AND Count = (SELECT MAX(Count) FROM DepartamentoGarantias);

/*25*/
SELECT Nombre, Apellido1, Apellido2, cliente.idCliente
FROM pepega_autoshop.diagnostico, pepega_autoshop.vehiculo, pepega_autoshop.cliente
WHERE diagnostico.Placa = vehiculo.Placa AND cliente.idCliente = vehiculo.idCliente
...
SELECT PromosCantidades.idPromo, Cantidad, Descripcion
FROM PromosCantidades, pepega_autoshop.promociones
WHERE PromosCantidades.idPromo = promociones.idPromo
HAVING Cantidad = (SELECT MAX(Cantidad)
FROM PromosCantidades AS Subconsulta2);

/*27*/
SELECT idPromo, Descripcion FROM pepega_autoshop.promociones
...
SELECT idEmpleadoR, Nombre, Apellido1, Apellido2, CantidadG 
FROM GarantiasAplicadas, pepega_autoshop.empleado
WHERE GarantiasAplicadas.idEmpleadoR = empleado.Matricula
AND CantidadG = (SELECT MAX(CantidadG)
...
SELECT cliente.Nombre, Apellido1, Apellido2, vehiculo.Placa, Servicios FROM VehiculoGanancia, pepega_autoshop.vehiculo, pepega_autoshop.cliente
