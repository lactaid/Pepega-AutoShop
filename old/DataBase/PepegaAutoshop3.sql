...
CREATE TABLE IF NOT EXISTS `pepega_autoshop`.`Trabajo` (
  ...
  INDEX `DiagnosticoTrabajo_idx` (`idDiagnostico` ASC) VISIBLE,
  ...
  CONSTRAINT `EmpleadoTrabajo`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `DiagnosticoTrabajo`
    FOREIGN KEY (`idDiagnostico`)
    REFERENCES `pepega_autoshop`.`Diagnostico` (`idDiagnostico`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
